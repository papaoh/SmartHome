...
class WeatherAnswerer(BaseAnswerer):
    ...
    def comparison_with_yesterday_form(self, location: str, date: str, result: dict) -> str:
        ...
        msg += '{date} {location}지역은 섭씨 {temperature}도이며, 어제보다 {comparison}. {weather}.' \
    ...
    def morning_afternoon_form(self, location: str, date: str, result: dict, josa: list) -> str:
        ...
        msg += '{date} {location}지역은 오전에{j1} 섭씨 {t1}도이며, {w1} ' \
               '오후에{j2} 섭씨 {t2}도이며, {w2} 강수확률은 오전은{percent_morning}이며, 오후는 {percent_afternoon}입니다.' \
            .format(date=date, location=location,
                    j1=josa[0], j2=josa[1],
                    ...
                    percent_morning=result['percent_morning'],
                    percent_afternoon = result['percent_afternoon']
                    )
